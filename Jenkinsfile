pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'hypersranshi'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/ranshi-farhan/SonarQube-Project-Kastro.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Ranshi -Dsonar.projectKey=RanshiKey -Dsonar.java.binaries=target"
                }
            }
        }
